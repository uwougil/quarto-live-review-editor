...
| 需求 | 实现 |
|---|---|
| 独立值域与 10% 步进 | `src/shared/documentZoom.ts` |
| 单一快捷键事件 owner 与 Ctrl/Mod 平台规则 | `src/webview-editor/documentZoom.ts` |
| 根节点 CSS 状态与 CodeMirror measure | `src/webview-editor/main.ts`、`media/webview-editor-theme.css` |
...
- [x] `Ctrl/Mod +`/`-` 只改变 reading width（60%–180%）。
...
- `npm test`：39 个测试文件、464 个测试通过。
- `npm run typecheck`：通过。
- `npm run compile`：通过。
- Chromium：`test:browser`、`test:browser:geometry`、`test:browser:inline`、`test:browser:inline-interaction`、`test:browser:typewriter`、`test:browser:arrow-scroll`、`test:browser:zoom`、`test:browser:math` 全部通过，且无页面错误。
